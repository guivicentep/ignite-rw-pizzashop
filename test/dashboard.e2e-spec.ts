...
test('display month canceled orders amount metric', async ({ page }) => {
  ...
  expect(page.getByText('19', { exact: true })).toBeVisible()
  expect(
...
test('display month revenue metric', async ({ page }) => {
  ...
  expect(page.getByText('R$ 50,00', { exact: true })).toBeVisible()
  expect(
    page.getByText('+12% em relação ao mês passado', { exact: true }),
  ).toBeVisible()
})
